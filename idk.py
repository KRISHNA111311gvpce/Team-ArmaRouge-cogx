a="install the packages using pip install streamlit google-generativeai plotly pandas"
...
with st.sidebar:
    ...
    # Button to open Google website
    if st.button("🌐 Ask IBM Granite 3.2.2b", help="Click to open Google in a new tab"):
        st.markdown(
            """
            <script>
    ...
    if api_key and st.session_state.user_profile:
        ...
        with col_a:
            if st.button("📋 Generate Budget Summary"):
                st.session_state.selected_action = "budget"
        
        with col_b:
            if st.button("💡 Spending Insights"):
                st.session_state.selected_action = "insights"
        
        with col_c:
            if st.button("🎯 Goal Planning"):
                st.session_state.selected_action = "goals"
        
        with col_d:
            if st.button("📈 Investment Advice"):
                st.session_state.selected_action = "investment"
        ...
        # Display results based on selected action
        if st.session_state.selected_action == "budget" and st.session_state.financial_data:
            with st.spinner("Creating personalized budget summary..."):
                ...
                Keep response under 300 words and adjust communication style for a {profile.get('user_type', 'general user')}.
                """
                ...
                try:
                    ...
                    st.markdown(f'<div class="budget-summary"><h4>📋 Budget Summary</h4>{response.text}</div>', unsafe_allow_html=True)
                except Exception as e:
                    st.error(f"Error generating budget summary: {e}")
        
        elif st.session_state.selected_action == "insights" and st.session_state.financial_data:
            with st.spinner("Analyzing spending patterns..."):
                profile = st.session_state.user_profile
                financial_data = st.session_state.financial_data
                
                prompt = f"""
                Analyze spending patterns for a {profile.get('user_type', 'user')}:
                
                Monthly Income: ₹{financial_data.get('monthly_income', 0)}
                Expense Breakdown: {json.dumps(financial_data.get('expenses', {}), indent=2)}
                
                Provide concise insights (under 250 words):
                1. Top spending categories
                2. 2-3 cost-cutting suggestions
                3. Spending efficiency score
                
                Use tone appropriate for a {profile.get('user_type', 'general user')}.
                """
        ...
        elif st.session_state.selected_action == "goals":
            with st.spinner("Creating personalized goal plan..."):
                profile = st.session_state.user_profile
                financial_data = st.session_state.financial_data
                
                prompt = f"""
                Create a concise financial goal plan for a {profile.get('user_type', 'user')}:
                
                Available monthly savings: ₹{financial_data.get('monthly_income', 0) - financial_data.get('total_expenses', 0)}
                Goals: {', '.join(profile.get('financial_goals', []))}
                Risk tolerance: {profile.get('risk_tolerance', 'unknown')}
                
                For top 3 priority goals, provide (under 250 words):
                1. Monthly allocation amount
                2. Estimated timeline
                3. Best strategy
                
                Adjust for {profile.get('user_type', 'general user')}.
                """
                
                try:
                    response = model.generate_content(prompt)
                    st.markdown(f'<div class="feature-card"><h4>🎯 Goal Planning</h4>{response.text}</div>', unsafe_allow_html=True)
                except Exception as e:
                    st.error(f"Error generating goal plan: {e}")
        
        elif st.session_state.selected_action == "investment":
            with st.spinner("Generating investment recommendations..."):
                ...
                prompt = f"""
                Provide concise investment advice for a {profile.get('user_type', 'user')}:
                - Monthly savings: ₹{available_savings}
                - Risk tolerance: {profile.get('risk_tolerance', 'unknown')}
                - Age: {profile.get('age_group', 'unknown')}
                
                Provide (under 250 words):
                1. Asset allocation %
                2. Top 3 investment products for India
                3. Monthly SIP recommendation
                
                Use terminology for {profile.get('user_type', 'general user')}.
                """
        ...
        elif st.session_state.selected_action and not st.session_state.financial_data:
            st.warning("⚠️ Please fill in your financial information in the 'Income & Expenses' section above to get personalized advice.")
